...
export default function EmailSection() {
  ...
  const openTemplateModal = () => { if (!quillRef.current) return; const html = quillRef.current.root.innerHTML.trim(); if (!html || html === "<p><br></p>") return alert("Message is empty!"); setShowTemplateForm(true); };
  const saveTemplate = () => {
    ...
    try {
      ...
      setTemplates([defaultTemplate, ...updated]); setTemplateName(""); setTemplateVisibility("admin"); setShowTemplateForm(false);
      alert("Template saved successfully!");
    } catch (error) { alert("Error saving template."); }
  };
  ...
  const sendEmail = () => {
    ...
    const newEmail = { id: crypto.randomUUID(), from: from || "(no sender)", to: toEmail, subject: subject || "(no subject)", message: quillRef.current.root.innerHTML, status: "Sent", date: new Date().toLocaleString("en-GB") };
    try {
      const updated = [newEmail, ...emailLogs];
      setEmailLogs(updated);
      localStorage.setItem("emailLogs", JSON.stringify(updated)); resetForm(); alert("Email Sent Successfully!");
    } catch (error) { alert("Error sending email."); }
  };
  const deleteLog = (id) => { if (window.confirm("Delete this email?")) { const updated = emailLogs.filter(log => log.id !== id); setEmailLogs(updated); localStorage.setItem("emailLogs", JSON.stringify(updated)); } };

  return (
    <div className="w-full bg-white">
      <style>{`@keyframes slideDown { from { opacity: 0; transform: translateY(-50px); } to { opacity: 1; transform: translateY(0); } }
        .animate-slideDown { animation: slideDown 0.3s ease-out; } .ql-container { font-family: inherit; }
        .ql-tooltip { left: auto !important; right: 0 !important; transform: none !important; }`}</style>

...
// const InputField = ({ label, name, value, onChange, type = "text", placeholder, readOnly, icon, className = "" }) => (
//     <div className={className}>
//         <label className="block text-sm text-gray-600 mb-2 flex items-center gap-2">
//             {label} {icon}
//         </label>
//         {readOnly ? (
//             <div className="text-gray-600 py-1">{value || placeholder}</div>
//         ) : (
//             <input
//                 type={type}
//                 name={name}
...
//                 className={`w-full text-black px-4 py-2 border border-gray-300 rounded hover:bg-gray-50 focus:outline-none focus:border-gray-400 ${readOnly ? 'bg-gray-100' : ''}`}
//             />
//         )}
//     </div>
// );
...
//         <div className="border-b border-gray-200 px-6 py-4">
//             <h2 className="text-lg font-normal text-gray-700" dangerouslySetInnerHTML={{ __html: title }} />
//         </div>
...
//     <div className="border-t bg-gray-100 border-gray-200 px-4 sm:px-6 py-3 flex flex-col-reverse sm:flex-row justify-end gap-3">
//         <button onClick={onSave} className="w-full sm:w-auto px-8 py-2 text-white bg-cyan-500 rounded hover:bg-cyan-600">Save</button>
//         <button onClick={onCancel} className="w-full sm:w-auto px-6 py-2 text-gray-600 bg-white border border-gray-300 rounded hover:bg-gray-50">Cancel</button>
//     </div>
// );
...
// export default function AdminProfile() {
//     const [formData, setFormData] = useState({
//         userName: '', firstName: '', lastName: '', designation: '', country: '', 
//         countryCode: '', contactNo: '', gender: '', profileImage: null
//     });
//     const [emailUpdates, setEmailUpdates] = useState(true);
//     const [reminderMandatory, setReminderMandatory] = useState('Yes');
//     const [showSuccess, setShowSuccess] = useState(false);
...
//     const handleCancel = () => confirm('Unsaved changes will be lost. Continue?');
//     const handleChangePassword = () => alert('Change password functionality will be implemented');
//     const handleEmailUpdate = () => alert('Email preferences updated successfully!');
//     const handleDeleteAccount = () => confirm('Are you sure you want to delete your account? This action cannot be undone.') && alert('Account deletion process initiated');
//     const handleReminderDateUpdate = () => {
//         setShowSuccess(true);
//         setTimeout(() => setShowSuccess(false), 3000);
//     };

//     const emailIcon = <svg className="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
//         <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
//     </svg>;
...
//                     <div className="bg-white rounded-lg shadow-sm">
//                         <div className="border-b border-gray-300 px-4 sm:px-6 py-3 sm:py-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
//                             <h1 className="text-lg sm:text-xl font-normal text-gray-700">Admin <span className="font-semibold">Profile</span></h1>
//                             <button onClick={handleChangePassword} className="w-full sm:w-auto px-6 py-2 text-white bg-gray-700 rounded hover:bg-gray-800">
//                                 Change Password
//                             </button>
//                         </div>
...
//                                 <InputField label="User Name" name="userName" value={formData.userName} onChange={handleInputChange} placeholder="User Name" className="md:col-span-4" />
//                                 <div className="md:col-span-4">
//                                     <InputField label="Email" icon={emailIcon} value="and.test.21990@gmail.com" readOnly />
//                                 </div>
//                                 <div className="md:col-span-2">
//                                     <label className="block text-sm text-gray-600 mb-1">Profile Image</label>
//                                     <div className="flex flex-col sm:flex-row gap-1 items-start">
//                                         <input type="file" accept="image/*" onChange={handleImageChange} 
//                                             className="text-sm file:px-1 file:py-1 file:border text-black file:bg-white file:text-gray-700 hover:file:bg-gray-50" />
//                                         <div className="w-20 h-20 border border-gray-300 rounded hover:bg-gray-50 overflow-hidden flex items-center justify-center flex-shrink-0">
...
//                             <div className="grid grid-cols-1 md:grid-cols-12 gap-4 sm:gap-6 mb-4 sm:mb-5">
//                                 <InputField label="First Name" name="firstName" value={formData.firstName} onChange={handleInputChange} placeholder="First Name" className="md:col-span-4" />
//                                 <InputField label="Last Name" name="lastName" value={formData.lastName} onChange={handleInputChange} placeholder="Last Name" className="md:col-span-4" />
//                                 <RadioGroup label="Gender" name="gender" value={formData.gender} onChange={handleInputChange} 
//                                     options={[{ value: 'male', label: 'Male' }, { value: 'female', label: 'Female' }]} className="md:col-span-4" />
//                             </div>

//                             <div className="grid grid-cols-1 md:grid-cols-12 gap-4 sm:gap-6 mb-4 sm:mb-6">
//                                 <InputField label="Designation" name="designation" value={formData.designation} onChange={handleInputChange} placeholder="Designation" className="md:col-span-4" />
//                                 <div className="md:col-span-4">
//                                     <label className="block text-sm text-gray-600 mb-2">Country</label>
//                                     <select name="country" value={formData.country} onChange={handleInputChange} 
//                                         className="w-full text-black px-4 py-2.5 border border-gray-300 rounded focus:outline-none focus:border-gray-400">
//                                         <option value="">Select Country</option>
//                                         {['India', 'United States', 'United Kingdom', 'Canada', 'Australia'].map(c => <option key={c} value={c}>{c}</option>)}
//                                     </select>
//                                 </div>
//                             </div>
...
//                                 <div className="md:col-span-2">
//                                     <label className="block text-sm text-gray-600 mb-2">Country Code</label>
//                                     <input type="text" name="countryCode" value={formData.countryCode} onChange={handleInputChange} placeholder="+91" readOnly
//                                         className="w-40 text-black px-4 py-2 border border-gray-300 rounded bg-gray-100 focus:outline-none" />
//                                 </div>
//                                 <InputField label="Contact No" name="contactNo" type="tel" value={formData.contactNo} onChange={handleInputChange} placeholder="Contact No" className="w-43 md:col-span-4" />
//                             </div>
//                         </div>
//                         <ButtonGroup onSave={handleSave} onCancel={handleCancel} />
//                     </div>

//                     <Section title="Daily <span class='font-semibold'>Email Status</span>" className="mt-6">
//                         <div className="flex items-center gap-3 mb-4">
...
//                             </label>
//                         </div>
//                         <button onClick={handleEmailUpdate} className="px-6 py-2 text-white bg-cyan-500 rounded hover:bg-cyan-600">Update</button>
//                     </Section>

//                     <Section title="Delete <span class='font-semibold'>Account</span>" className="mt-6">
//                         <button onClick={handleDeleteAccount} className="px-6 py-2 text-white bg-red-500 rounded hover:bg-red-600">Delete Account</button>
//                     </Section>

//                     <Section title="Manage <span class='font-semibold'>Reminder Date</span>" className="mt-6">
//                         {showSuccess && (
//                             <div className="mb-4 bg-teal-50 border border-teal-200 rounded px-4 py-3 flex items-center justify-between">
//                                 <p className="text-teal-700 text-sm">Success: The <span className="font-semibold">Record</span> has been Updated.</p>
//                                 <button onClick={() => setShowSuccess(false)} className="text-teal-500 hover:text-teal-700 text-xl leading-none">×</button>
//                             </div>
//                         )}
//                         <div className="mb-4">
//                             <RadioGroup label="Set Lead Remind Date Mandatory" name="reminderMandatory" value={reminderMandatory} 
//                                 onChange={(e) => setReminderMandatory(e.target.value)} 
//                                 options={[{ value: 'Yes', label: 'Yes' }, { value: 'No', label: 'No' }]} />
//                         </div>
//                         <button onClick={handleReminderDateUpdate} className="px-6 py-2 text-white bg-[#0aa699] rounded">Update</button>
//                     </Section>

//                     <div className="bg-white rounded-lg shadow-sm mt-6">
//                         <div className="border-b border-gray-200 px-6 py-4">
//                             <h2 className="text-lg font-normal text-gray-700">Your <span className="font-semibold">Timezone</span></h2>
//                         </div>
//                         <div className="mb-6">
//                             <p className="text-red-600 text-sm mt-4 ml-6">Your Current Timezone : <span className="font-semibold">Asia/Kolkata</span></p>
//                             <label className="block text-gray-700 text-sm font-medium mt-4 ml-6">Time Zone</label>
//                             <select className="w-60 text-black px-4 py-2.5 mt-4 ml-6 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-500"
//                                 value={timezone} onChange={(e) => setTimezone(e.target.value)}>
//                                 {['Asia/Kolkata', 'America/New_York', 'Europe/London', 'Asia/Tokyo'].map(tz => <option key={tz} value={tz}>{tz}</option>)}
//                             </select>
//                         </div>
//                         <ButtonGroup onSave={handleSave} onCancel={handleCancel} />
//                     </div>
